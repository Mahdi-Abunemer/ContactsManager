...
namespace CRUDExample.Filters.ResourceFilter
{
    public class FeatureDisabledResourceFilter : IAsyncResourceFilter
    {
        ...
        public async Task OnResourceExecutionAsync(ResourceExecutingContext context, ResourceExecutionDelegate next)
        {
            ...
            if (_isDisable)
            {
                //context.Result = new NotFoundResult(); //Not Found 404 ( the url will not be use any more )
                context.Result = new StatusCodeResult(501); // 501 ( temprorey disable the action method )
            }
            else
            {
                await next();
